...
import { SimulationService, PhaseConfiguration } from '../../../simulation-service';
...
stopSimulation() {
    this.isLoading = true;
    this.updateMessage('Arrêt de la simulation SUMO en cours...');

    this.simulationService.stopSimulation().subscribe({
      next: (response) => {
        this.updateMessage('✅ Simulation SUMO arrêtée avec succès !');
        this.simulationRunning = false;
        this.stopDynamicDataRefresh();
        this.isLoading = false;
      },
      error: (err) => {
        this.handleError('❌ Erreur lors de l\'arrêt de la simulation SUMO', err);
        this.isLoading = false;
      },
    });
  }
...
setPriorityDirection(dir: string) {
  ...
  getCurrentPhase(): number {
    return (this.traffic_light.phases[0].currentPhaseIndex) + 1;
    console.log('Current Phase Index:', this.traffic_light?.phase.currentPhaseIndex);
  }
...
isOnChildRoute(): boolean {
  // Cette méthode vérifie si on est sur une route enfant
  // Pour l'instant, retournez false pour toujours afficher la simulation
  // Vous pourrez l'améliorer plus tard avec le service Router
  return false;
}
